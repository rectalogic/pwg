...
def make_ipv4(proto, src_ip, dst_ip, body):
    ip_header = bytearray(
        struct.pack(">BxH2s2xBB2x4s4s", 0x45, len(body) + 20, os.urandom(2), 64, proto, src_ip.packed, dst_ip.packed)
    )
...
class TCPStack:
    ...
    def send(self, tcp_body=b"", *, flag=Control.ACK, seq=None, ack=None):
        ...
        tcp_header = struct.pack(
            ...
            self.dst_port,
            self.src_port,
            (self.dst_seq if seq is None else seq) & 0xFFFFFFFF,
            (self.src_seq if ack is None else ack) & 0xFFFFFFFF,
            5 << 4,
            flag,
            window,
            0,
        ...
        tochecksum = bytearray(
            self.dst_ip.packed + self.src_ip.packed + b"\x00\x06" + len(ip_body).to_bytes(2, "big") + ip_body
        )
...
class IPPacket:
    def __init__(self, dns: str, verbose: int | None):
        self.tcp_stack = {}
        self.dns_server = dns
        ...
        if proto == enums.IpProto.UDP:
            ...
            if tcp is None:
                ...
                self.tcp_stack[key] = tcp = TCPStack(
                    src_ip, src_port, dst_ip, dst_name, dst_port, reply, option, self.verbose
                )
